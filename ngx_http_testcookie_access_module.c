...
//        return NGX_DECLINED;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "vietnix args len: %d", args->len);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "vietnix ctx ok: %d", ctx->ok);
    if (ctx->ok == 1) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                "user passed test");
        if(args->len == 0){
          ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                "vietnix kiem tra testcookie_arg");
          return NGX_DECLINED;
        }
    }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix case%d", sc);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix l: %d", l);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix k: %d", k);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix attempts: %d\n", attempt);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix buf len: %d", len);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix buf before: redirectig user to %s", buf);
    if (ctx->ok == 1) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                "user passed test");
        if(attempt == 0){
    ...
    if (look->len > 0) {
        ...
        case 4:
            	attempt++;
		if(ctx->ok == 1){
			int temp_len_rm = k - 2 - look->len;
			if(temp_len_rm >= 0){
            			p = ngx_copy(p, args->data, temp_len_rm);
			}
                        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix case4 len rm:  %d", temp_len_rm);
		}else{
			(*p++) = '?';
...
redirect:
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix buf len: %d", len);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "vietnix buf after: redirectig user to %s", buf);
